...
<div class="fig" style="min-width:320px"><div class="lbl">背驰后三种转折（第29课）</div><div style="display:flex;gap:10px;align-items:flex-start">${drawZS([{p:13},{p:8,tag:'底',label:'一买'},{p:10,label:'触DD=10',color:'#f59e0b',above:true}], [{lo:11,hi:13,x0:0,x1:2,label:'中枢B'}], {w:26,h:92})}${drawZS([{p:13},{p:8,tag:'底',label:'一买'},{p:12,label:'回中枢',color:'#2563eb',above:true}], [{lo:11,hi:13,x0:0,x1:2,label:'中枢B'}], {w:26,h:92})}${drawZS([{p:13},{p:8,tag:'底',label:'一买'},{p:16,label:'反趋势',color:'#16a34a',above:true}], [{lo:11,hi:13,x0:0,x1:2,label:'中枢B'}], {w:26,h:92})}</div><ol class="turn"><li><b>级别扩展</b>：反弹最弱，只触及 B中枢 DD=10</li><li><b>更大级别盘整</b>：反弹回 B中枢，横向扩展</li><li><b>以上级别反趋势</b>：反弹强，直接转上涨</li></ol><div class="cap">判别关键：看反弹的第一个次级别走势<br>是否<b>回抽进最后一个中枢</b></div></div>`;
  ...
  __chapters.push({
    id: 'ch14', vol: '卷四 · 背驰与买卖点', title: '第14章 背驰', source: '原文第24、29、37、43课',
    figures: [
    ...
    sections: [
      ...
      { type: 'evolution', title: '背驰：从 MACD 辅助到严格结构判定', text: '背驰是缠论动力学核心，它的判定标准被缠师一路收紧：从第24课的“MACD 辅助”起步，到第37课给出严格的结构前提，再到第43课把转折与级别的关系讲透。', items: [
        { term: '① 第24课（2007-01）：MACD 辅助判断', text: '第一次系统讲背驰，用的是 <b>MACD 辅助</b>：两段同向趋势比较，最后一个中枢把黄白线回拉 0 轴附近，c 段柱面积小于 b 段。同时明确：<span class="hl">“但光用 MACD 辅助判断，即使你对中枢不大清楚，只要能判断背驰与盘整背驰，准确率 90% 以上；配合中枢，就是 100% 绝对”</span>。', fig: mfig('第24课：MACD 辅助', drawZS([{ p: 8 }, { p: 13, tag: '顶' }, { p: 10 }, { p: 14 }, { p: 11 }, { p: 17, tag: '顶' }, { p: 14, label: 'c(面积小)', color: '#e74c3c', above: true }], [{ lo: 10, hi: 13, x0: 0, x1: 3, label: 'A' }, { lo: 14, hi: 17, x0: 4, x1: 6, label: 'B(回拉0轴)' }], { w: 26, h: 88, zgzd: true }), '黄白线回拉 0 轴 + c 面积 < b') },
        { term: '② 第37课（2007-03）：背驰的再分辨', text: '第37课把背驰的前提收紧到严格结构：<b>a+A+b+B+c</b> 中的背驰，必须是在<b>趋势</b>（至少两个同向中枢）里比较 c 与 b；<span class="hl">“没有趋势，没有背驰”</span>。同时区分了中枢扩展、盘整等易与背驰混淆的情形——不是两段力度变小就叫背驰。', fig: mfig('第37课：没有趋势没有背驰', drawZS([{ p: 8 }, { p: 12 }, { p: 9 }, { p: 11 }, { p: 14 }, { p: 17 }, { p: 15 }, { p: 18 }, { p: 20, label: 'c', color: '#e74c3c', above: true }], [{ lo: 9, hi: 11, x0: 0, x1: 3, label: 'A' }, { lo: 15, hi: 17, x0: 4, x1: 7, label: 'B' }], { w: 24, h: 88 }), '趋势中比较 c 与 b，没趋势不背驰') },
        { term: '③ 第43课（2007-04）：背驰-转折与级别', text: '第43课《有关背驰的补习课》把背驰与转折的关系讲细：<span class="hl">背驰导致转折，转折的级别一定等于或小于背驰所在走势的级别</span>。也就是说，某级别背驰引发的转折，级别不会超过这个走势级别本身——这给“背驰后怎么转、转多大”定了一个上界。', fig: mfig('第43课：转折级别 ≤ 背驰级别', '<div style="font-size:12px;line-height:1.9;color:#1f2937">背驰 → 转折<br><b style="color:#d97706">转折级别 ≤ 背驰走势级别</b><br>（不会超过走势级别本身）</div>', '背驰引发的转折，级别有上界') },
      ]},
      { type: 'quote', title: '背驰的原话金句', text: '背驰最核心的一句话，看似简单，却是绝大多数人出错的地方：', quotes: [
        { text: '没有趋势，没有背驰。', src: '第37课' },
        { text: 'c 必然包含对 B 的一个第三类买卖点，否则就只能看成 B 中枢的小级别波动。', src: '第37课' },
      ]},
    ],
  });
})();
