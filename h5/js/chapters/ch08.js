  ...
  __chapters.push({
    ...
    sections: [
      { type: 'definition', title: '特征序列与划分标准', items: [
        { term: '① 特征序列（第67课）', plain: plainHTML('白话：向上线段里，那些「往回走」的<b>向下笔</b>（X1、X2…），就是它的特征序列；向下线段里则是它的向上笔。一句话——<b>与线段方向相反的笔</b>。'), analogy: analogyHTML('上台阶前的下蹲', '每上一级台阶前，脚会先往下蹲一下再发力。向上线段里那一次次「下蹲」（向下笔），就是特征序列——它最能体现这段走势的「劲儿」。'), text: '以向上笔开始的线段用笔序列 <code>S1X1S2X2S3X3…SnXn</code> 表示（S 向上、X 向下）。定义序列 <code>X1X2…Xn</code> 为<span class="hl">向上线段</span>的<b>特征序列</b>；序列 <code>S1S2…Sn</code> 为<span class="kw">向下线段</span>的特征序列。即：<b>特征序列 = 与线段方向相反的笔</b>。', fig: mfig('特征序列 = 反向笔', biLineSVG([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 12, tag: '底' }, { p: 18, tag: '顶' }, { p: 13, tag: '底' }], { segColors: ['#2563eb', '#f59e0b', '#2563eb', '#f59e0b'], w: 50, h: 110 }), '向上线段中，向下笔 X1、X2 即特征序列') },
        { term: '② 缺口（第67课）', plain: plainHTML('两个相邻特征序列元素的区间<b>没重叠、中间留空</b>，就叫缺口。'), analogy: analogyHTML('两节车厢没挂钩', '两节车厢之间没扣上挂钩，中间空了一段——这个空档就是「缺口」。'), text: '特征序列<b>两相邻元素间没有重合区间</b>，称为该序列的一个<span class="hl">缺口</span>。（因为反向笔之间可能不重叠，这正体现了特征序列更能代表线段性质。）', fig: mfig('缺口：两元素无重合区间', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 17, hi: 20, label: 'X2' }], { w: 50, h: 100 }), 'X1 顶 16 < X2 底 17 → 之间留缺口') },
        { term: '③ 标准特征序列（第67课）', plain: plainHTML('把每个特征元素当成一根 K 线，像普通 K 线那样做<b>包含处理</b>，得到「标准特征序列」。以后说特征序列都指它。'), text: '把特征序列的<b>每一元素看成一根 K 线</b>，像普通 K 线找分型那样做<b>包含处理（非包含处理）</b>，得到<span class="hl">标准特征序列</span>。以后凡说特征序列，都指标准特征序列。', fig: mfig('标准特征序列（含包含处理）', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { w: 46, h: 100 }), '把元素当 K 线做包含处理 → 标准序列') },
        { term: '④ 只考察一种分型（第67课）', plain: plainHTML('向上线段<b>只找顶分型</b>，向下线段<b>只找底分型</b>——方向别搞反。'), text: '参照顶/底分型定义确定特征序列的顶和底。注意：<b>向上线段的特征序列只考察顶分型；向下线段只考察底分型</b>。', fig: mfig('向上线段只考察顶分型', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { w: 46, h: 100 }), '向上线段：特征序列只找顶分型') },
        { term: '⑤ 第一种情况（无缺口，第67课）', plain: plainHTML('无缺口 → 干净利落，线段<b>直接在该分型结束</b>（顶分型在最高点、底分型在最低点）。'), text: '特征序列的<b>顶分型</b>中，第一和第二元素间<b>不存在</b>特征序列缺口 → 该线段在该顶分型的<b>高点</b>处结束；<b>底分型</b>第一二元素间无缺口 → 在该底分型的<b>低点</b>处结束。', fig: mfig('第一种情况：无缺口', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { w: 46, h: 100 }), 'X1、X2 有重叠（无缺口）→ 线段在该分型结束') },
        { term: '⑥ 第二种情况（有缺口，第67课）', plain: plainHTML('有缺口 → 不能当场定，要<b>等反向序列再出一个相反分型</b>才确认线段结束；且第二个分型<b>不分第一、二种情况，只要有分型就行</b>。'), text: '顶分型第一二元素间<b>存在</b>缺口 → 若从该分型最高点开始的向下一笔序列的特征序列<b>出现底分型</b>，则该线段在该顶分型高点处结束；底分型有缺口 → 若反向序列出现顶分型，则在该底分型低点处结束。<b>强调</b>：第二种情况下，后一特征序列不一定封闭前一缺口，且第二个序列中的分型<b>不分第一二种情况，只要有分型即可</b>。', fig: mfig('第二种情况：有缺口', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 17, hi: 20, label: 'X2' }, { lo: 14, hi: 19, label: 'X3' }], { w: 46, h: 100 }), 'X1、X2 留缺口 → 需看反向序列是否出分型') },
        { term: '⑦ 包含关系的前提（第71课）', plain: plainHTML('不同特征序列的元素之间，讨论包含关系<b>没意义</b>——只有同一序列里的元素才能比包含。'), text: '特征序列的元素要讨论<b>包含关系</b>，首先必须是<b>同一特征序列</b>的元素；两个不同特征序列之间的元素讨论包含关系<b>没有意义</b>。', fig: mfig('同一序列才可讨论包含', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { w: 46, h: 100 }), '不同特征序列之间的元素讨论包含无意义') },
        { term: '⑧ 当下划分程序（第71课）', tree: treeHTML({
          start: 'n0',
          nodes: {
            n0: { q: '假设某转折点是线段的终点。先看：特征序列分型 第一、二元素间 有无缺口？', opts: [
              { t: '无缺口（两元素区间有重叠）', next: 'case1' },
              { t: '有缺口（中间留空）', next: 'case2' },
            ]},
            case1: { result: 'green', t: '第一种情况 → 线段在该分型的高点（顶分型）/ 低点（底分型）【直接结束】。' },
            case2: { q: '有缺口（第二种情况）。再看：从该分型极值点开始的反向序列，是否出现「相反分型」？', opts: [
              { t: '出现相反分型', next: 'end' },
              { t: '没有出现', next: 'cont' },
            ]},
            end: { result: 'green', t: '出现相反分型 → 线段在该分型极值点【结束】。这个分型不分第一、二种情况，只要有分型即可。' },
            cont: { result: 'red', t: '没有出现 → 原线段【延续】，该假设转折点不是真正的终点。' },
          }
        }), text: '线段的划分可<b>当下完成</b>：假设某转折点是两线段分界点，用两种划分情况去考察——<b>满足其一，即为真正的分界点；都不满足，则原线段延续</b>。', fig: mfig('当下划分：满足其一即分界', biLineSVG([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 12, tag: '底' }, { p: 18, tag: '顶' }], { w: 50, h: 110 }), '用两种划分情况考察转折点，满足其一即真正分界') },
