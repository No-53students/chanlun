  ...
  __chapters.push({
    ...
    sections: [
      { type: 'definition', title: '中枢的定义与区间', items: [
        ...
        { term: '⑧ 动手练：标出中枢的 ZG / ZD（第20课）', text: '三段次级别走势 A[10,16]、B[11,15]、C[12,17]。中枢区间是<b>三个低点里最高的</b>（<span class="kw">ZD</span>=12）到<b>三个高点里最低的</b>（<span class="hl">ZG</span>=15）。标出 ZG、ZD 对应的那两个点。点击节点切换标记（ZG→ZD→清除），标完点「判分」。', draw: drawHTML({
          kind: 'line',
          title: '动手标 ZG / ZD',
          intro: '点击折线节点，标出「ZG（上沿=min高点）」和「ZD（下沿=max低点）」。',
          marks: ['ZG', 'ZD'],
          pts: [
            { p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 11, tag: '底' }, { p: 15, tag: '顶' }, { p: 12, tag: '底' }, { p: 17, tag: '顶' },
          ],
          answer: { 3: 'ZG', 4: 'ZD' },
        }) },
      ]},
      ...
      { type: 'evolution', title: '中枢定义：从“三段重叠”到数学公式', text: '中枢的定义不是一次成型的：第17课先给出定性描述，第20课才补上 <b>ZG/ZD/GG/DD</b> 的精确数学表达式，把“重叠”落到可计算的区间上。', items: [
        { term: '① 第17课（2006-12）：首次定性定义', text: '<span class="hl">“某级别走势类型中，被至少三个连续次级别走势类型所重叠的部分，称为缠中说禅走势中枢。”</span>这一句同时点出两个关键：① 是<b>次级别走势类型</b>的重叠（不是本级别 K 线）；② 存在<b>递归</b>问题（次级别不能无限分下去，最低级别改用单位 K 线重叠）。', fig: figDef },
        { term: '② 第20课（2007-01）：数学精确化', text: '定性定义不够用，第20课补上可计算的表达：设三段次级别走势 A、B、C 的高/低点，中枢区间 = <code>[max(三个低点), min(三个高点)]</code>。再定义 <span class="kw">ZG=min(g1,g2)</span>（上沿）、<span class="kw">ZD=max(d1,d2)</span>（下沿）、<b>GG/DD</b>（中枢内最高/最低）。从此“是否还在中枢里”变成一道<b>纯算术</b>题。', formula: '中枢区间 = [ max(低点), min(高点) ]　ZG=min(g1,g2) ZD=max(d1,d2)', fig: figZGZD },
        { term: '③ 由精确化直接导出：级别延续与第三类买卖点', text: '有了精确区间，第20课才可能进一步推出<b>级别延续定理一、二</b>（更大级别中枢何时产生）和<b>第三类买卖点定理</b>（离开后回试不破 ZG / 不升破 ZD）。可见：<span class="hl">定义越精确，能推的定理越多</span>——这是缠论“像几何一样”的典型示范。', fig: figBS3 },
      ]},
    ],
  });
})();
