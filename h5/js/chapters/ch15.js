  ...
  __chapters.push({
    ...
    sections: [
      { type: 'definition', title: '买卖点的完备性', items: [
        ...
        { term: '③ 买卖点与中枢的关系（第21课）', tree: treeHTML({
          start: 'n0',
          nodes: {
            n0: { q: '判断买点（以买点为例，卖点对称相反）。先看：当前是否处于下跌趋势的背驰点？', opts: [
              { t: '是，下跌趋势背驰点（中枢之下）', next: 'buy1' },
              { t: '不是，继续看', next: 'n1' },
            ]},
            buy1: { result: 'green', t: '第一类买点（中枢之下，转折）。由下跌趋势背驰精确制造。' },
            n1: { q: '第一类买点之后，现在是不是「第一次次级别回调」的低点？', opts: [
              { t: '是', next: 'buy2' },
              { t: '不是，继续看', next: 'n2' },
            ]},
            buy2: { result: 'green', t: '第二类买点（位置任意）。一买后第一次次级别回调低点，绝对安全。' },
            n2: { q: '是否「次级别离开中枢后，回试不跌破 ZG」？', opts: [
              { t: '是（离开中枢 + 回试不破 ZG）', next: 'buy3' },
              { t: '否', next: 'none' },
            ]},
            buy3: { result: 'green', t: '第三类买点（中枢之上，新生/扩张）。其后必然赢利。' },
            none: { result: 'amber', t: '不符合三类买点任何一条 → 不是被理论保证的买点，别硬找。' },
          }
      ...
      { type: 'definition', title: '三类买点详解', items: [
        ...
        { term: '⑦ 动手练：标出三类买点（第17/20/21课）', text: '下面这段「下跌→背驰→转折→上涨」的走势里，藏着<span class="hl">第一、二、三类买点</span>各一个。点击折线节点切换标记（一买→二买→三买→清除），标完点「判分」。', draw: drawHTML({
          kind: 'line',
          title: '动手标三类买点',
          intro: '点击折线节点，标出「一买 / 二买 / 三买」（再点切换、点第四下清除）。',
          marks: ['一买', '二买', '三买'],
          pts: [
            { p: 20, tag: '顶' }, { p: 15 }, { p: 18 }, { p: 14 }, { p: 17 },
            { p: 11 }, { p: 14 }, { p: 10 }, { p: 13 }, { p: 8, tag: '底' },
            { p: 13 }, { p: 10, tag: '底' }, { p: 14 }, { p: 12 }, { p: 16 },
            { p: 13 }, { p: 17 }, { p: 14, tag: '底' },
          ],
          zones: [
            { lo: 15, hi: 17, x0: 1, x1: 4, label: '中枢A' },
            { lo: 11, hi: 13, x0: 5, x1: 8, label: '中枢B' },
            { lo: 13, hi: 14, x0: 12, x1: 15, label: '中枢C' },
          ],
          answer: { 9: '一买', 11: '二买', 17: '三买' },
        }) },
      ]},
