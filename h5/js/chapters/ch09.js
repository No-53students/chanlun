...
<div class="fig" style="min-width:250px"><div class="lbl bad">② 有缺口（第二种情况）</div>${gapSeqSVG([{ lo: 12, hi: 16, label: '第1·X1' }, { lo: 17, hi: 20, label: '第2·X2' }, { lo: 14, hi: 19, label: 'X3' }], { i: 0, lo: 16, hi: 17, label: '缺口', color: '#e74c3c' }, { w: 54, h: 128 })}<div class="cap">X1 上沿 16 &lt; X2 下沿 17（<b>有缺口</b>）<br>需看反向序列是否出现底分型</div></div>`;
  ...
  const figMid = `
<div class="fig" style="min-width:250px"><div class="lbl">延伸三笔 → 第三笔破第一笔结束位置</div>${drawZS([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 11, tag: '底' }, { p: 14, tag: '顶' }, { p: 10.5, tag: '底', label: '新线段', color: '#9333ea' }], [{ lo: 10.5, hi: 16, x0: 0, x1: 3, label: '第一笔范围 [10,16]' }], { w: 46, h: 130 })}<div class="cap">破坏那笔（顶→11）是转折点第一笔<br>延伸三笔、第三笔破第一笔结束位 16 → 新线段成立</div></div>
<div class="fig" style="min-width:250px"><div class="lbl">第三笔完全在第一笔范围内</div>${drawZS([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 13, tag: '底' }, { p: 15, tag: '顶' }, { p: 13.5, tag: '底' }], [{ lo: 10, hi: 16, x0: 0, x1: 4, label: '第一笔范围 [10,16]' }], { w: 46, h: 130 })}<div class="cap">第三笔(15)未破 16、也未破 10 → 分不出方向<br>先破 16 → 新线段；先破 10 → 旧线段延续</div></div>`;
  ...
  __chapters.push({
    ...
    sections: [
      { type: 'definition', title: '缺口判定的唯一标准', items: [
        { term: '① 唯一判断标准（第71课）', text: '67 课把线段划分分为两种情况，<span class="hl">分清楚是哪种情况，对划分线段十分关键</span>。判断的标准<b>只有一个</b>：特征序列的分型中，<b>第一和第二元素间是否存在特征序列的缺口</b>。无缺口即第一种情况，有缺口即第二种情况。', fig: mfig('判断标准唯一', gapSeqSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 17, hi: 20, label: 'X2' }, { lo: 14, hi: 19, label: 'X3' }], { i: 0, lo: 16, hi: 17, label: '缺口', color: '#e74c3c' }, { w: 46, h: 110 }), '只看第一、二元素间有无缺口') },
        { term: '② 第一元素与第二元素（第71课）', text: '特征序列分型中，<span class="kw">第一元素</span>就是<b>假设转折点前线段的最后一个特征元素</b>；<span class="kw">第二元素</span>就是<b>从这转折点开始的第一笔</b>。显然这两者<b>方向相同</b>，因此：两者之间有缺口 → 第二种情况；无缺口 → 第一种情况，然后按定义考察即可。', fig: mfig('第一、二元素', intervalsSVG([{ lo: 12, hi: 16, label: '第1' }, { lo: 17, hi: 20, label: '第2' }, { lo: 14, hi: 19, label: '第3' }], { w: 46, h: 110 }), '第1＝转折点前最后特征元素，第2＝转折点第一笔') },
        { term: '③ 第一种情况·无缺口（第67课）', text: '特征序列的<b>顶分型</b>中，第一、第二元素间<b>不存在</b>缺口 → 该线段在顶分型的<b>高点</b>处结束；<b>底分型</b>第一、二元素间无缺口 → 在该底分型的<b>低点</b>处结束。', fig: mfig('第一种情况：无缺口', gapSeqSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { i: 0, lo: 13, hi: 16, label: '有重叠', color: '#16a34a' }, { w: 46, h: 110 }), 'X1、X2 重叠（无缺口）→ 线段直接结束') },
        { term: '④ 第二种情况·有缺口（第67课）', text: '顶分型第一、二元素间<b>存在</b>缺口 → 若从该分型最高点开始的向下一笔序列的特征序列<b>出现底分型</b>，则该线段在该顶分型高点处结束；底分型有缺口 → 反向序列出现顶分型则在该底分型低点结束。<span class="hl">强调</span>：后一特征序列<b>不一定封闭前一缺口</b>，且第二个序列的分型<b>不分第一、二种情况，只要有分型即可</b>。', fig: mfig('第二种情况：有缺口', gapSeqSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 17, hi: 20, label: 'X2' }, { lo: 14, hi: 19, label: 'X3' }], { i: 0, lo: 16, hi: 17, label: '缺口', color: '#e74c3c' }, { w: 46, h: 110 }), '有缺口 → 需反向序列出现相反分型确认') },
        { term: '⑤ 包含关系的前提（第71课）', text: '特征序列元素要讨论<b>包含关系</b>，首先前提是这些元素<b>都在同一特征序列里</b>；两个不同特征序列之间的元素，讨论包含关系<b>没意义</b>。特征序列元素的方向与其对应段方向<b>刚好相反</b>（向上段的特征序列元素向下、向下段的向上），所以不同段之间根本不可能存在包含。', fig: mfig('同一序列才可讨论包含', intervalsSVG([{ lo: 12, hi: 16, label: 'X1' }, { lo: 13, hi: 17, label: 'X2' }, { lo: 11, hi: 16, label: 'X3' }], { w: 46, h: 110 }), '不同特征序列之间的元素，讨论包含无意义') },
        { term: '⑥ 转折点前后两元素无包含（第71课）', text: '在假设的转折点<b>前后那两个元素</b>，是<b>不存在包含关系</b>的——因为这两者已被假设为不同性质的东西，不一定是同一特征序列。但假设转折点<b>之后</b>的顶分型元素，<b>可以应用包含关系</b>（它们必是同一类的东西：或同属原线段特征序列，或同属新线段非特征序列）。', fig: mfig('前后两元素不含、后元素可含', gapSeqSVG([{ lo: 12, hi: 16, label: '前·X1' }, { lo: 13, hi: 17, label: '后·X2' }, { lo: 11, hi: 16, label: 'X3' }], { i: 0, lo: 13, hi: 16, label: '后元素间可含', color: '#2563eb' }, { w: 46, h: 110 }), '转折点前后两元素无包含；其后元素间可包含') },
      ]},
      { type: 'definition', title: '三种情形与当下划分', items: [
        ...
        { term: '③ 最早破坏那笔就是转折点第一笔（第71课）', text: '若最早破坏那笔<b>就是转折点下来的第一笔</b>，这笔就属于<b>「中间地带」</b>——既不能说它是前一段特征序列，更不能说是后一段特征序列，处于<b>待定</b>状态。此时即使出现看似特征序列包含关系的走势，也不能算。它一旦<b>延伸出三笔以上，新的线段就形成</b>，那时再谈前线段特征序列的包含关系就没意义了。', fig: mfig('中间地带：待定', biLineSVG([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 11, tag: '底' }, { p: 14, tag: '顶' }, { p: 10.5, tag: '底' }], { segColors: ['#2563eb', '#9333ea', '#9333ea', '#9333ea'], w: 46, h: 110 }), '破坏那笔（紫）是转折点第一笔 → 待定，延伸三笔方成新线段') },
        { term: '④ 第三笔完全在第一笔范围内（第71课）', text: '更复杂的情况：<b>第三笔完全在第一笔的范围内</b>，这三笔就分不出向上还是向下，也就<b>定义不了特征序列</b>（特征序列与走势相反，走势连方向都没有，就无从判断）。结果无非两种：<span class="kw">1</span> 先破第一笔的<b>结束位置</b> → 新线段成立、旧线段被破坏；<span class="kw">2</span> 先破第一笔的<b>开始位置</b> → 旧线段只被一笔破坏后延续原方向，新线段不出现。', fig: mfig('第三笔在第一笔范围内', drawZS([{ p: 10, tag: '底' }, { p: 16, tag: '顶' }, { p: 13, tag: '底' }, { p: 15, tag: '顶' }, { p: 13.5, tag: '底' }], [{ lo: 10, hi: 16, x0: 0, x1: 4, label: '第一笔范围 [10,16]' }], { w: 40, h: 104 }), '方向未定：先破 16 成新线段，先破 10 旧线段延续') },
